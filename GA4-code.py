...
# Access data
original = spark.read.csv("gs://ibd-ga4/original.csv", header = True, inferSchema = True)
updated = spark.read.csv("gs://ibd-ga4/updated.csv", header = True, inferSchema = True)

for row in updated.collect():
  condition1 = F.col("name") == row.name
  condition2 = F.col("end_date") > current_date

  original = original.withColumn("end_date", when(condition1 & condition2, current_date).otherwise(original.end_date))

for row in updated.collect():
  idx_ = original.tail(1)[0].idx + 1
  name_ = row.name
  dob_ = row.dob
  tuple_ = (idx_, name_, dob_, current_date, "10-12-2077")
